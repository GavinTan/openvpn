...
> 6. 服务端若通过 NAT/端口映射等方式暴露到外网，可设置环境变量 `OVPN_REMOTE_ADDR`（外显外网 IP）与 `OVPN_REMOTE_PORT`（外显外网端口），生成的客户端 .ovpn 中的 `remote` 行会使用这两个值；留空则自动探测本机出网 IP 与配置端口。例：`OVPN_REMOTE_ADDR=203.0.113.10`、`OVPN_REMOTE_PORT=1194`。
