...
> 5. 默认生成的证书一年有效期，如果想要设置更长时间的有效期可以添加环境变量EASYRSA_CA_EXPIRE、EASYRSA_CERT_EXPIRE、EASYRSA_CRL_DAYS设置来覆盖默认设置，例：EASYRSA_CA_EXPIRE=3650。
> 6. 服务端若通过 NAT/端口映射等方式暴露到外网，可在「系统设置-openvpn」配置「外显IP」「外显端口」，生成的客户端 .ovpn 中的 `remote` 行会使用这两个值；留空则自动探测本机出网 IP 与配置端口。也可用环境变量 `OVPN_REMOTE_ADDR`、`OVPN_REMOTE_PORT` 作为首次部署的初始值（之后以 Web 配置为准）。
> 7. 通知邮件相关：「系统设置-邮件配置」可设「邮件带登录链接」开关（默认开，关闭则邮件不含登录链接）、「使用说明链接」（附在所有通知邮件底部，留空不显示）。登录链接取自「系统设置-基本-站点URL」，也可用环境变量 `OVPN_SITE_URL`（登录链接）、`OVPN_EMAIL_LOGIN_LINK`（开关，true/false）、`OVPN_HELP_URL`（使用说明）作为初始值。
