...
(function (factory) {
  ...
  $.fn.dataTableExt.oApi.fnColReorder = function (oSettings, iFrom, iTo, drop, invalidateRows) {
    ...
    $(oSettings.oInstance).trigger('column-reorder.dt', [
      oSettings,
      {
        from: iFrom,
        to: iTo,
  ...
  $.extend(ColReorder.prototype, {
    ...
    _fnMouseDown: function (e, nTh, i) {
      ...
      var col = parseInt($(nTh).attr('data-column-index'), 10);
      if (!isNaN(col)) {
        i = col;
      }
    ...
    _fnMouseMove: function (e, colResized) {
      ...
      if (this.dom.resize) {
        ...
        for (
          ...
          currentColumnIndex < this.s.dt.aoColumns.length - 1 && currentColumnIndex != colResized;
          currentColumnIndex++
        ) {
        ...
        if (!scrollXEnabled && !scrollYEnabled) {
          if (moveLength < 0) {
            ...
            if (nThNextInnerWidth <= minNThNextWidth) {
              return;
            }
        ...
        // Smart scrolling is enabled.
          ...
          if (moveLength < 0) {
            ...
            if ($scrollBodyNThNext.width() <= minNThNextWidth) {
              return;
            }
          ...
          if (
            Math.pow(
              Math.pow(this._fnCursorPosition(e, 'pageX') - this.s.mouse.startX, 2) +
                Math.pow(this._fnCursorPosition(e, 'pageY') - this.s.mouse.startY, 2),
              0.5
            ) < 5
          ) {
            return;
        ...
        for (var i = 1, iLen = this.s.aoTargets.length; i < iLen; i++) {
          if (
            this._fnCursorPosition(e, 'pageX') <
    ...
    _fnMouseUp: function (e, colResized) {
      ...
      if (this.dom.drag !== null) {
        ...
        if (!scrollXEnabled) {
          ...
          for (
            nextVisibleColumnIndex = colResized + 1;
            nextVisibleColumnIndex < this.s.dt.aoColumns.length;
          ...
          for (
            previousVisibleColumnIndex = colResized - 1;
            previousVisibleColumnIndex >= 0;
        ...
        if (scrollXEnabled && $(this.s.dt.nScrollHead, this.s.dt.nTableWrapper) != undefined) {
          if ($(this.s.dt.nScrollHead, this.s.dt.nTableWrapper).length > 0)
            this.table_size = $(
              $(this.s.dt.nScrollHead, this.s.dt.nTableWrapper)[0].childNodes[0].childNodes[0]
            ).width();
        }
    ...
    _fnCreateDragNode: function () {
      ...
      this.dom.drag = $(origTable.cloneNode(false))
        ...
        .append($(origThead.cloneNode(false)).append($(origTr.cloneNode(false)).append(cloneCell[0])))
        .css({
          position: 'absolute',
          top: 0,
          left: 0,
